...
def cnn_train():
    ...
    X_train, y_train = [], []
    for i in range(n):
        img = cv2.imdecode(np.fromfile(train_path + pic_name[i], dtype=np.uint8), -1)  # cv2.imshow无法读取中文路径图片，改用此方式
    ...
    n = len(pic_name)
    for i in range(n):
        img = cv2.imdecode(np.fromfile(val_path + pic_name[i], dtype=np.uint8), -1)  # cv2.imshow无法读取中文路径图片，改用此方式
        label = [char_dict[name] for name in pic_name[i][0:7]]  # 图片名前7位为车牌标签
        X_val.append(img)
        y_val.append(label)
    ...
    Output = [layers.Dense(68, activation='softmax', name='c%d' % (i + 1))(x) for i in range(7)]  # 7个输出分别对应车牌7个字符，每个输出都为68个类别类概率
    ...
    if os.path.exists(ckpt_path):
        latest = tensorflow.train.latest_checkpoint(ckpt_path)
        if latest != None:
